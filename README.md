...
The camera does not provide verified absolute PTZ coordinates. Calibration drives the camera to its left and down physical stops, establishing a repeatable reference. Because V380 direction commands are short steps rather than continuous motor-start commands, the decoder refreshes horizontal commands every 250 ms and vertical commands every 100 ms. `pan` and `tilt` record command windows from the reference corner, not degrees or camera-reported coordinates.
...
All timed movement requests require a duration from 250 to 10000 milliseconds in 250 ms increments. The decoder refreshes the direction command throughout that window and then sends the camera stop command. A failed, cancelled, or manually overridden move is not added to the logical position.
